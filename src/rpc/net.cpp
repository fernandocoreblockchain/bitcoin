...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    ...
    for (const CNodeStats& stats : vstats) {
        ...
        obj.pushKV("atommagic", stats.fUsesAtomMagic);
...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:7733\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:7733\", \"onetry\"")
        );
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:7733\",  (string) The bitcoin fercoin server IP and port we're connected to\n"
